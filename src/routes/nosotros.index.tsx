    ...
    <div className="min-h-screen bg-background text-foreground antialiased">
      ...
      <main>
        <SystemPageHero
          ...
          context="Diseñamos y operamos sistemas que convierten demanda en ventas: campañas, conversaciones, CRM e IA supervisada, medidos hasta el ingreso. Trabajamos con empresas consolidadas que ya venden e invierten en mercadeo, y pierden dinero entre la campaña y el cierre. Operamos en Latinoamérica desde Colombia: Bogotá, Medellín y Barranquilla."
        ...
        <section className="page-section"><div className="page-shell"><SectionHeader num="04." label="Principios" title={<>La base común de <span className="text-orange">toda cuenta.</span></>} /><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{PRINCIPIOS.map(([n,Icon,title,text]) => <article key={n} className="content-card p-7"><div className="flex items-center gap-3"><Icon className="h-6 w-6 text-orange" /><span className="font-hero text-[28px] font-semibold text-orange">{n}</span></div><h3 className="font-display mt-4 text-[19px] font-semibold">{title}</h3><p className="mt-2 text-[15px] leading-relaxed text-muted-foreground">{text}</p></article>)}</div></div></section>
        <section className="page-section"><div className="page-shell"><SectionHeader num="05." label="A quién servimos" title="A quién servimos." /><p className="mt-8 max-w-[900px] text-[18px] leading-[1.7] text-muted-foreground">Nuestro cliente es una empresa consolidada que ya vende, ya invierte en mercadeo o ventas, y pierde dinero entre la campaña y el cierre. El tamaño no es un filtro absoluto: una empresa de 12 personas con ticket alto y margen es mejor cliente que una de 80 con márgenes bajos.</p></div></section>
        <section className="page-section"><div className="page-shell grid items-stretch gap-6 md:grid-cols-2"><LinkCard title="Cómo trabajamos" text="Operar, Sprint o Partner: las modalidades y cómo crece una cuenta." href="/nosotros/como-trabajamos" /><LinkCard title="Mercados" text="Dónde operamos: Latinoamérica, desde Colombia." href="/mercados/" /></div></section>
        <GeneralCta />
      </main><SiteFooter />
    </div>
  );
