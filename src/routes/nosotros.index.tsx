...
const principios = [
  { icon: <Database size={20} />, titulo: "Una fuente de verdad", detalle: "Un solo modelo de datos: pauta → prospecto → MQL → SQL → cita → oportunidad → venta → margen, con definiciones que el cliente firma." },
  { icon: <ShieldCheck size={20} />, titulo: "IA supervisada", detalle: "Cada cuenta documenta qué se automatiza, qué requiere aprobación humana, cómo se detectan fallos y quién interviene." },
  { icon: <UserRoundCheck size={20} />, titulo: "Un responsable con autoridad", detalle: "Decide prioridades entre pauta, creatividad, conversión y operación. No coordina: responde por el resultado." },
  { icon: <Layers size={20} />, titulo: "Activos reutilizables", detalle: "Conectores, tracking, evaluaciones, playbooks y biblioteca creativa. Lo que se repite se documenta y se versiona." },
  { icon: <Lock size={20} />, titulo: "Gobierno y seguridad", detalle: "Accesos, datos personales, consentimiento y cumplimiento local, con apoyo jurídico cuando haga falta." },
  { icon: <PackageOpen size={20} />, titulo: "Transferencia", detalle: "Documentación y accesos completos desde el primer día. El cliente puede irse con su sistema." },
];
  ...
  return (
    <div className="rckt-site tcn-page">
      <main id="top">
        ...
        <SystemSection num="01." label="Manifiesto" title={<>Lo que <em className="font-serif-accent">creemos</em>.</>}>
          <blockquote className="home-proof-quote nosotros-quote">
            Hacemos crecer lo que importa, aplicamos inteligencia con precisión, medimos lo que hacemos y
            construimos relaciones que perduran, porque el verdadero crecimiento no se persigue: se diseña,
            se demuestra y se sostiene.
          </blockquote>
        </SystemSection>
        ...
        <SystemSection num="03." label="Identidad" title={<>Lo que somos y lo que <em className="font-serif-accent">no</em>.</>}>
        ...
        <SystemSection num="04." label="Los 5 pilares" title={<>Cinco <em className="font-serif-accent">pilares</em>.</>}>
          ...
          <div className="system-highlight" style={{ marginTop: 40 }}>
            <p style={{ fontSize: "clamp(17px,2vw,20px)", fontWeight: 500, lineHeight: 1.6 }}>
              <strong>En la práctica.</strong> Vender mejor es decirle que no al prospecto que no encaja,
              aunque duela el mes. Pensar mejor es medir antes de tocar nada. Trabajar más inteligente es un
              proceso a la vez, con supervisión, no un chatbot suelto. Movernos primero es que el Diagnostic
              dure semanas, no trimestres. Y la prueba por encima de la promesa es que ningún resultado se
              menciona sin ficha de caso.
            </p>
          </div>
        </SystemSection>

        <SystemSection num="05." label="Principios" title={<>Seis <em className="font-serif-accent">principios</em> que no negociamos.</>}>
          <CapabilityCards items={principios} />
        </SystemSection>

        <SystemSection num="06." label="A quién servimos" title={<>A quién <em className="font-serif-accent">servimos</em>.</>}>
          <p className="system-section__lead">
            Trabajamos con empresas consolidadas que ya venden, ya invierten en pauta o ventas y pierden dinero
            entre la campaña y el cierre. El tamaño no es un filtro absoluto: una empresa de 12 personas con
            ticket alto y buen margen puede ser mejor cliente que una de 80 con márgenes bajos.
          </p>
          <article className="combo-card combo-card-light nosotros-link-card" style={{ marginTop: 40 }}>
            ...
            <a href="/nosotros/como-trabajamos" className="combo-card__link">Ver cómo trabajamos →</a>
          </article>
        </SystemSection>
