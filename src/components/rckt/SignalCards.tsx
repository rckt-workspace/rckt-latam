...
export default function SignalCards({ items }: { items: Signal[] }) {
  ...
  return (
    <div className="signal-grid">
      {grid.map((signal, index) => (
        <SignalCard key={signal.titulo} n={index + 1} signal={signal} />
      ))}
      {wide.map((signal, index) => (
        <SignalCard key={signal.titulo} n={grid.length + index + 1} signal={signal} wide />
      ))}
    </div>
  );
}
