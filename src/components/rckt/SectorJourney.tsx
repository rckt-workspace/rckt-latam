...
export default function SectorJourney({ stages, leaks = [], icons }: { stages: string[]; leaks?: JourneyLeak[]; icons?: LucideIcon[] }) {
  const { ref, inView } = useInView<HTMLDivElement>({ threshold: 0.15, fallbackMs: 900 });
  const set = icons ?? DEFAULT_ICONS;
  const total = stages.length;
  return (
    <div ref={ref} className="journey" data-in={inView}>
      <div className={`journey__line ${inView ? "is-in" : ""}`} aria-hidden="true" />
      <div className="journey__row">
        {stages.map((stage, i) => {
          const Icono = set[i % set.length];
          const leak = leaks.find((l) => l.afterStage === i + 1);
          const delay = (900 / Math.max(total - 1, 1)) * i;
          return (
            <div key={stage} className="journey__stage">
              <div className={`journey__dot ${inView ? "is-in" : ""}`} style={{ transitionDelay: `${delay}ms` }}>
                <Icono className="h-5 w-5" strokeWidth={1.6} aria-hidden="true" />
              </div>
              <p className="journey__name">{stage}</p>
              {leak ? (
                <div className={`journey__leak ${inView ? "is-in" : ""}`} style={{ transitionDelay: `${900 + i * 90}ms` }}>
